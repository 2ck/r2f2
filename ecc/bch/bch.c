/*
 * Modified by Yannick Loeck <y@2ck.me>, Jul 2026:
 *  - make suitable for use outside of Linux kernel
 *  - fix signed shift overflows
 */
...
#define GF_POLY_SZ(_d)                                                         \
    (sizeof(struct gf_poly) + ((_d) + 1) * sizeof(unsigned int))
...
static inline unsigned int gf_div(struct bch_control *bch, unsigned int a,
                                  unsigned int b) {
    return a ? bch->a_pow_tab[mod_s(bch, bch->a_log_tab[a] + GF_N(bch) -
                                             bch->a_log_tab[b])]
             : 0;
}
...
static int find_poly_deg2_roots(struct bch_control *bch, struct gf_poly *poly,
    ...
    if (poly->c[0] && poly->c[1]) {
        ...
        if ((gf_sqr(bch, r) ^ r) == u) {
            ...
            roots[n++] =
                modulo(bch, 2 * GF_N(bch) - l1 - bch->a_log_tab[r] + l2);
            roots[n++] =
                modulo(bch, 2 * GF_N(bch) - l1 - bch->a_log_tab[r ^ 1] + l2);
        }
    }
    return n;
}
 ...
 */
static void gf_poly_logrep(struct bch_control *bch, const struct gf_poly *a,
                           int *rep) {
